...
import com.Crud.SpringbootApplication.Model.Reports;
...
public class ReportsController {
  ...
  public ResponseEntity<ResponseMessage> uploadReports(@RequestParam("reports") MultipartFile reports) {
    String message = "";
    try {
    	reportsStorageService.save(reports);

      message = "Uploaded the Report successfully: " + reports.getOriginalFilename();
      return ResponseEntity.status(HttpStatus.OK).body(new ResponseMessage(message));
    } catch (Exception e) {
      message = "Could not upload the Report: " + reports.getOriginalFilename() + ". Error: " + e.getMessage();
  ...
  @GetMapping("/reportss")
  public ResponseEntity<List<Reports>> getListReports() {
    List<Reports> reports = reportsStorageService.loadAll().map(path -> {
      String filename = path.getFileName().toString();
      String url = MvcUriComponentsBuilder
          .fromMethodName(ReportsController.class, "getReports", path.getFileName().toString()).build().toString();
      
      return new Reports(filename, url);
  ...
  @GetMapping("/reportss/{filename:.+}")
  @ResponseBody
  public ResponseEntity<Resource> getReports(@PathVariable String filename) {
    Resource reports = reportsStorageService.load(filename);
    return ResponseEntity.ok()
        .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + reports.getFilename() + "\"").body(reports);
  }
